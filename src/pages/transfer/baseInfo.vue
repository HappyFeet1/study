...
export default {
    ...
    methods:{
        initData(){
            var cache = JSON.parse(localStorage.getItem('$$transferDetailCache'));
            if(!cache){
                this.$toast('查询无结果！');
                return;
            }
            this.borrowerInfo = cache.borrowerInfo||{};
            this.creditRecord = cache.creditRecord||{};
            this.detailInfo = cache.loanInfo.projectDetails + cache.loanInfo.riskDetails;
        }
    },
    beforeMount(){
        this.initData();
