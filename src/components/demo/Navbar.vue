<template>
    <div class="page-navbar">
        <mt-header title="Navbar">
            <div slot="left">
                <mt-button icon="back" @click="$router.back()">返回</mt-button>
            </div>
        </mt-header>
        <mt-navbar class="page-part" v-model="selected">
            <mt-tab-item id="1">选项一</mt-tab-item>
            <mt-tab-item id="2">选项二</mt-tab-item>
            <mt-tab-item id="3">选项三</mt-tab-item>
        </mt-navbar>
    
        <div>
            <mt-cell class="page-part" title="当前选中">{{ selected }}</mt-cell>
        </div>
    
        <mt-tab-container v-model="selected">
            <mt-tab-container-item id="1">
                <mt-cell v-for="n in 10" :key="n" :title="'内容 ' + n" />
            </mt-tab-container-item>
            <mt-tab-container-item id="2">
                <mt-cell v-for="n in 4" :key="n" :title="'测试 ' + n" />
            </mt-tab-container-item>
            <mt-tab-container-item id="3">
                <mt-cell v-for="n in 6" :key="n" :title="'选项 ' + n" />
            </mt-tab-container-item>
        </mt-tab-container>
    </div>
</template>

<script>
import Navbar from '@/ui/navbar/index.vue';
import TabItem from '@/ui/tab-item/index.vue';
import Cell from '@/ui/cell/';
import TabContainer from '@/ui/tab-container/index.vue';
import TabContainerItem from '@/ui/tab-container-item/index.vue';
export default {
    name: 'page-navbar',
    data() {
        return {
            selected: '1'
        };
    },
    components: {
        'mt-navbar': Navbar,
        'mt-tab-item': TabItem,
        'mt-cell': Cell,
        'mt-tab-container':TabContainer,
        'mt-tab-container-item':TabContainerItem
    }
};
</script>